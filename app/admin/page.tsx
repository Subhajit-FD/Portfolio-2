...
export default function Admin() {
  ...
  const uploadImage = async (file: File, type: "mobile" | "desktop") => {
    ...
    try {
      ...
      toast.error(err.message || `Failed to upload ${type} image`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDeleteProject = async (id: string) => {
    ...
    try {
      ...
      toast.error(err.message || "Failed to delete project", { id: deleteToast });
    }
  };
  ...
  const handleToggleFeatured = async (proj: ProjectItem) => {
    ...
    try {
      ...
      // Revert UI on error
      setProjects(prev => prev.map(p => p._id === proj._id ? { ...p, featured: isCurrentlyFeatured } : p));
      toast.error(err.message || "Failed to update carousel preference");
    }
  };
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || `Failed to ${isEditing ? "update" : "create"} project`, { id: submitToast });
    } finally {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-background text-foreground md:flex-row">
      ...
      {/* Sidebar Panel */}
      <aside className="flex w-full shrink-0 flex-col border-b border-border bg-card p-6 md:w-64 md:border-r md:border-b-0 md:h-screen md:sticky md:top-0">
        ...
        <nav className="flex flex-col gap-2 grow">
          <button
            ...
            className={`flex items-center gap-3 px-4 py-2.5 text-sm font-semibold tracking-wide transition-all border-l-2 ${
              activeTab === "list"
        ...
        <div className="mt-auto border-t border-border pt-4 flex flex-col gap-3">
          ...
          <Button 
            ...
            className="w-full flex items-center justify-center gap-2 hover:bg-destructive hover:text-destructive-foreground hover:border-destructive transition-colors text-xs py-2 h-auto"
          >
            <SignOut size={16} />
            <span>Log Out</span>
          </Button>
        ...
        <header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h2 className="text-2xl font-heading font-black tracking-tight uppercase md:text-3xl">
              {activeTab === "list" ? "Projects Catalog" : editingProjectId ? "Edit Project" : "Add Project"}
            </h2>
          ...
          {activeTab === "list" && (
            <Button 
              ...
              className="flex items-center gap-2 text-xs py-2 h-auto"
            >
              <Plus size={16} />
          ...
          <div className="bg-card border border-border">
            {isProjectsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center px-4">
                ...
                <Button onClick={() => setActiveTab("create")} className="text-xs py-2 h-auto">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-border bg-muted/40 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                      ...
                      <th className="px-6 py-4 hidden sm:table-cell">Desktop Screen</th>
                      <th className="px-6 py-4 hidden sm:table-cell">Mobile Screen</th>
                      <th className="px-6 py-4">Show in Carousel</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {projects.map((proj) => (
                      <tr key={proj._id} className="hover:bg-muted/10 transition-colors text-xs">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            {proj.liveUrl ? (
                              ...
                              <span className="text-muted-foreground/30"><Globe size={16} /></span>
                            )}
                            ...
                            {proj.githubUrl ? (
                              ...
                              <span className="text-muted-foreground/30"><GithubLogo size={16} /></span>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end items-center gap-2">
                            <Button
                              ...
                              className="h-8 w-8 hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors text-muted-foreground"
                              title="Edit Project"
                            >
                            ...
                            <Button
                              ...
                              className="h-8 w-8 hover:bg-destructive hover:text-destructive-foreground hover:border-destructive transition-colors text-muted-foreground"
                      ...
                      <div className="relative w-full aspect-video overflow-hidden border border-border bg-black">
                        ...
                        {isDesktopUploading ? (
                          ...
                          <div className="absolute inset-0 bg-black/75 flex flex-col items-center justify-center p-4 text-center text-destructive">
                            <XCircle size={24} weight="fill" className="mb-1" />
                      ...
                      <div className="relative h-[180px] aspect-[9/16] overflow-hidden border border-border bg-black">
                        ...
                        {isMobileUploading ? (
                          ...
                          <div className="absolute inset-0 bg-black/75 flex flex-col items-center justify-center p-2 text-center text-destructive">
                            <XCircle size={20} weight="fill" className="mb-1" />
              ...
              <div className="flex items-center gap-4 pt-4 border-t border-border mt-8">
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      {editingProjectId ? <CheckCircle size={16} /> : <Plus size={16} />}
